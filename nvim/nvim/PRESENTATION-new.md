...
5. Move to next word, press `.` to repeat the surround
6. Undo all: `u` until restored
...
3. Move to next name, press `.` to repeat the replacement
4. Undo with `u`
...
## 13. Autocompletion with blink.cmp

- Auto-show is off — ghost text shows inline preview instead
- `<C-Space>` to manually trigger the completion menu
- `<CR>` to accept, `<C-e>` to dismiss
- Signature help shows function params as you type
- Smart sources: LSP in code, buffer-only in comments

---

### Try it: blink.cmp

1. Open `UserService.scala`, start typing `def get` — see ghost text appear
2. Press `<C-Space>` to open the menu, browse with arrows, `<CR>` to accept
3. Type `findUser(` — signature help shows parameter info
4. Add a comment `//` and press `<C-Space>` — only buffer words appear
...
- `:Jq` formats JSON in the current buffer using `jq`
- Supports Vim ranges to format only specific lines
- Defined as a custom command in `lua/config/commands.lua`

---

### Range Examples

- `:%Jq` — format the entire buffer
- `:'<,'>Jq` — format visually selected lines
- `:10,20Jq` — format lines 10 through 20
- `:.Jq` — format the current line only
- `:.,.+5Jq` — format from current line to 5 lines below

---

### Try it: :Jq

1. Open a file with inline JSON (or paste `{"name":"alice","age":30,"roles":["admin","user"]}`)
2. Run `:%Jq` to pretty-print the entire buffer
3. Visually select a few JSON lines with `V`, then type `:Jq`
4. Try `:1,3Jq` to format only lines 1–3
