...
Validate that workDir is a safe, expected path.

```javascript
/**
 * Validate workDir is within expected scratchpad directory
 * @param {string} path - Path to validate
 * @returns {string} - Validated path
 * @throws {Error} - If path is invalid or unsafe
 */
function validateWorkDir(path) {
  if (typeof path !== 'string' || path.length === 0) {
    throw new Error('workDir must be a non-empty string');
  }
  if (path.includes('\0')) {
    throw new Error('workDir contains null bytes');
  }
  if (path.includes('..')) {
    throw new Error('workDir must not contain path traversal (..)');
  }
  if (!path.includes('.workflow/.scratchpad/review-code-')) {
    throw new Error('workDir must be within .workflow/.scratchpad/review-code- namespace');
  }
  return path;
}
```
...
function updateState(workDir, updates) {
  ...
  try {
    ...
    // Clean up temp file (use Write instead of shell to avoid injection)
    Write(tempPath, '');
  } catch (error) {
...
function restoreState(workDir, backupPath = null) {
  ...
  // Validate backupPath stays within workDir
  if (backupPath && !backupPath.startsWith(workDir)) {
    throw new Error('backupPath must be within workDir');
  }
