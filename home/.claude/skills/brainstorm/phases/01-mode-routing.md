...
if (execution_mode === null) {
  AskUserQuestion({
    questions: [{
      question: "Select brainstorm mode",
      header: "Mode Selection",
      multiSelect: false,
      options: [
        {
          label: "Auto Mode (Recommended)",
          description: "Full pipeline: Framework generation → Parallel multi-role analysis → Cross-role synthesis. Best for comprehensive analysis of new topics"
        },
        {
          label: "Single Role Analysis",
          description: "Generate analysis for a single role. Best for adding role perspectives to existing sessions or iterative updates"
        }
      ]
    }]
  });
  ...
  if (user_selected === "Auto Mode (Recommended)") {
    execution_mode = 'auto';
  } else {
    execution_mode = 'single-role';
  }
...
if (execution_mode === 'single-role' && !role_name) {
  ...
  AskUserQuestion({
    questions: [{
      question: "Select a role to analyze",
      header: "Role Selection",
      multiSelect: false,
      options: [
        { label: "system-architect", description: "System Architect - Technical architecture, scalability, integration patterns" },
        { label: "ux-expert", description: "UX Expert - User research, information architecture, user journeys" },
        { label: "product-manager", description: "Product Manager - Product strategy, roadmap, prioritization" },
        { label: "ui-designer", description: "UI Designer - Visual design, high-fidelity prototypes, design systems" }
      ]
    }]
  });
