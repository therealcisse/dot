...
1. **Original role outputs are immutable** — Role analysis documents represent each role's raw perspective, synthesis phase is read-only
2. **Spec is the final artifact** — All synthesis decisions, conflict resolutions, and user clarifications are reflected in the spec, never backfilled into role documents
3. **Single agent sequential generation** — One Spec Agent loads context once and generates all specs sequentially, cross-feature decisions carry forward
4. **On-demand validation** — Review Agent is triggered by Spec Agent based on complexity self-assessment, not a mandatory step
...
AskUserQuestion({
  questions: [{
    question: "Select enhancement recommendations to apply",
    header: "Enhancement Selection",
    multiSelect: true,
    options: [
      { label: "EP-001: API Contract", description: "Add detailed request/response schema definitions" },
      { label: "EP-002: User Intent", description: "Clarify user requirement priorities and acceptance criteria" }
    ]
  }]
})
...
AskUserQuestion({
  questions: [
    {
      question: "What is the core objective for the MVP phase?",
      header: "User Intent",
      multiSelect: false,
      options: [
        { label: "Rapid Validation", description: "Minimal feature set, ship fast for feedback" },
        { label: "Technical Foundation", description: "Solid architecture, build for long-term growth" },
        { label: "Feature Complete", description: "Cover all planned features, delayed launch" }
      ]
    }
  ]
})
...
1. **Actionable**: resolution must be directly executable. Bad: "Needs trade-off analysis" → Good: "Use JWT stateless auth, RefreshToken in HttpOnly Cookie"
2. **Justified**: Explain why. Format: "[Solution] because [reason], tradeoff: [cost]"
3. **Scoped**: If limited scope, mark "Applies when: [condition]"
...
**All interactions via AskUserQuestion (English questions)**
...
for (let i = 0; i < enhancements.length; i += BATCH_SIZE) {
  ...
  AskUserQuestion({
    questions: [{
      question: `Select enhancement recommendations to apply (Round ${Math.floor(i/BATCH_SIZE)+1})`,
      header: "Enhancement Selection",
      multiSelect: true,
      options: batch.map(ep => ({
        label: `${ep.id}: ${ep.title}`,
        description: `Impact: ${ep.affected_roles.join(', ')} | ${ep.enhancement}`
      }))
    }]
  })
...
// User can also skip: provide "Skip" option
```
...
**Question Structure**: `[Cross-role analysis finding] + [Decision point needing clarification]`
**Option Structure**: `Label: [Specific solution] + Description: [Business impact] + [Technical trade-off]`

**9-Category Taxonomy**:

| Category | Focus | Example Question Pattern |
|----------|-------|--------------------------|
| User Intent | User Goals | "MVP phase core objective?" + Validation/Foundation/Completeness |
| Requirements | Requirements Refinement | "How to prioritize features?" + Core/Enhancement/Optional |
| Architecture | Architecture Decisions | "Technology stack considerations?" + Familiarity/Innovation/Maturity |
| UX | User Experience | "Interaction complexity trade-offs?" + Simple/Rich/Progressive |
| Feasibility | Feasibility | "Scope under resource constraints?" + Minimal/Standard/Complete |
| Risk | Risk Management | "Risk tolerance?" + Conservative/Balanced/Aggressive |
| Process | Process Standards | "Iteration cadence?" + Fast/Stable/Flexible |
| Decisions | Decision Confirmation | "Conflict resolution?" + Approach A/Approach B/Compromise |
| Terminology | Terminology Alignment | "Which terminology to standardize on?" + Term A/Term B |

**Quality Rules**:

**MUST Include**:
- All questions in the user's language
- Based on specific cross-role analysis findings
- Options include business impact explanations
- Resolve actual ambiguities or conflicts

**MUST Avoid**:
- Generic questions unrelated to role analysis
- Repeating content already confirmed in the artifacts phase
- Overly detailed implementation-level questions
