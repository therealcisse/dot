---
name: brainstorm
description: Unified brainstorming skill with dual-mode operation — auto mode (framework generation, parallel multi-role analysis, cross-role synthesis) and single role analysis. Triggers on "brainstorm", "brainstorm session".
...
**Phase 1.5: Terminology & Boundary Definition**
   ...
   - Collect Non-Goals via AskUserQuestion (Explicitly excluded scope)
...
**Optional Review**: If 2+ prototypes and not `--yes`, execute multi-round interactive review with intelligent grouping.
